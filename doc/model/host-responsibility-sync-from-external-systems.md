# dayu-topology Host 责任关系外部系统同步设计

## 1. 文档目的

本文档定义 `dayu-topology` 中心侧如何从外部系统同步主机责任关系。
...
- `dayu-topology` 不直接把外部系统当查询数据库使用
- 外部系统数据应先同步到中心侧规范化对象
...
- 外部系统提供事实
- `dayu-topology` 负责归一、映射、继承和最终查询视图
